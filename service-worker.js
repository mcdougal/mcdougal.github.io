/* This file is not transpiled, so only use basic ES syntax features. */
...
const CACHE_NAME = 'cedricmcdougal/v1';

const DO_CACHE = [
  // Cache Next.js static files. Every production build uses unique filenames
  // so we don't have to worry about invalidating the cache. Example path:
  // /_next/static/runtime/main-d799a1982e05a8ca6951.js
  /^.*\/_next\/static\/.*\.js$/,
];
...
self.addEventListener('fetch', function (event) {
  const doCache = DO_CACHE.some(function (regex) {
    return regex.test(event.request.url);
  });

  const doNotCache = DO_NOT_CACHE.some(function (regex) {
    return regex.test(event.request.url);
  });

  if (doCache && !doNotCache) {
